...
public class Menu extends JPanel implements MouseListener, Runnable{
	...
	protected void paintComponent(Graphics g) {
        ...
        if(Start.press == true) {
        	this.repaint();
        	g.drawImage(Start.getImage_true(), 135, 350, 180, 65, null);
        }
        else{
        	this.repaint();
        	g.drawImage(Start.getImage_false(), 135, 350, 180, 65, null);
        }
        if(Continue.press == true) {
        	this.repaint();
        	g.drawImage(Continue.getImage_true(), 135, 420, 180, 65, null);
        }
        else {
        	this.repaint();
        	g.drawImage(Continue.getImage_false(), 135, 420, 180, 65, null);
        }
        if(Developer.press == true) {
        	this.repaint();
        	g.drawImage(Developer.getImage_true(), 135, 490, 180, 65, null);
        }
        else {
        	this.repaint();
        	g.drawImage(Developer.getImage_false(), 135, 490, 180, 65, null);
        }
	...
	public void mouseClicked(MouseEvent e) {
		...
		//Start
		if(e.getX() > 135 && e.getX() < 315 && e.getY() > 350 && e.getY() < 415){
		...
		//Continue
		else if(e.getX() > 135 && e.getX() < 315 && e.getY() > 420 && e.getY() < 485){
			...
			if(save == 0){
				int dialogButton = -1; 
				dialogButton = JOptionPane.showConfirmDialog(null, "Sorry, you don't have any record.","Confirm", dialogButton);
				if(dialogButton == JOptionPane.CLOSED_OPTION){
				}
			}
		}
		//Developer
		else if(e.getX() > 135 && e.getX() < 315 && e.getY() > 490 && e.getY() < 555){
	...
	public void mouseEntered(MouseEvent e) {
		...
		//Start
		/*if(e.getX() > 135 && e.getX() < 315 && e.getY() > 350 && e.getY() < 415){
			System.out.println("Start_In");
			Start.set_press(true);
		}
		//Continue
		else if(e.getX() > 135 && e.getX() < 315 && e.getY() > 420 && e.getY() < 485){
			System.out.println("Continue_In");
			Continue.set_press(true);
		}
		//Developer
		else if(e.getX() > 135 && e.getX() < 315 && e.getY() > 490 && e.getY() < 555){
			System.out.println("Developer_In");
			Developer.set_press(true);
		}
		else{
			System.out.println("nothing_In");
		}*/
	}
	@Override
